...
app.get("/api/settings", requireUser, (req,res) => res.json({ok:true,settings:req.user.settings}));
...
app.post("/api/ea/heartbeat", (req,res) => {
  if (API_SECRET && String(req.headers["x-api-secret"] || "") !== API_SECRET) return res.status(401).json({ok:false,error:"EA_AUTH_REQUIRED"});
  const accessKey = String(req.body?.accessKey || "").trim();
  if (!accessKey || !ACCESS_KEYS.includes(accessKey)) return res.status(403).json({ok:false,error:"INVALID_ACCESS_KEY"});
  const userId = tokenHash(accessKey).slice(0, 36);
  let user = [...users.values()].find(u => u.ea.keyHash === userId);
  if (!user) {
    user = makeUser();
    user.ea.keyHash = userId;
  }
  user.ea.online = true;
...
app.post("/api/ea/market", (req,res) => {
  ...
  if (!Number.isFinite(price) || price <= 0) return res.status(400).json({ok:false,error:"INVALID_PRICE"});
  const change = Number.isFinite(prev) && prev > 0 ? ((price-prev)/prev)*100 : 0;
  let signal="WAIT", score=0, reason="Conditions not aligned";
  if (Number.isFinite(rsi)) {
    if (change > 0.03 && rsi >= 55 && rsi <= 72) { signal="BUY"; score=75; reason="Positive momentum with bullish RSI confirmation"; }
...
app.get("/api/ea/command", (req,res) => {
  ...
  const allowed = s.autoTrade && ((signal==="BUY" && s.allowBuy && (s.direction==="BUY"||s.direction==="BOTH")) || (signal==="SELL" && s.allowSell && (s.direction==="SELL"||s.direction==="BOTH")));
  const room = Number(user.mt5.positions||0) < s.maxOpenTrades;
  const action = allowed && room ? signal : "WAIT";
  res.json({ok:true,action,takeProfit:s.takeProfit,maxOpenTrades:s.maxOpenTrades,positions:user.mt5.positions,signalScore:user.engine.score,reason: allowed ? (room ? user.engine.reason : "Maximum open trades reached") : "Signal blocked by user settings"});
});

app.get("/api/status", requireUser, (req,res)=>res.json({ok:true, user:safeUser(req.user), events:req.user.events.slice(0,20)}));
