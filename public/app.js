let token=sessionStorage.getItem("gold_automate_token")||"";
const $=id=>document.getElementById(id);
function api(path,opts={}){opts.headers={...(opts.headers||{}),...(token?{Authorization:"Bearer "+token}:{})};return fetch(path,opts).then(async r=>{const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.error||"REQUEST_FAILED");return d})}
function showApp(){ $("landing").classList.add("hidden");$("app").classList.remove("hidden");load(); }
